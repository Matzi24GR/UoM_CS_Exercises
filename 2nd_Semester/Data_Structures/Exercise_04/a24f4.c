...
int main() {
  ...
  do {
    scanf("%d", &listSize);
  } while (listSize<1 || listSize>NumberOfNodes);

  for (int i=0;i<listSize;i++){
    printf("Enter an integer: ");
    ListElementType element;
    scanf("%d", &element);
    Insert(&AList,Node,&FreePtr,-1,element);
  }
  ...
  if (choice==2) Ascending = FALSE;

  sort_list(&AList,Node,Ascending);
