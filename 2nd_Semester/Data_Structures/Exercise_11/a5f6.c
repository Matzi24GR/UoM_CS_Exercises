...
int main() {
  ...
  do {
    ...
    printf("USERNAME: ");
    scanf(" %s", &userElem.RecKey);

    printf("PASSWORD: ");
    scanf(" %s", &userElem.password);

    int Loc, Pred;
    SearchHashList(HashList, userElem.RecKey, &Loc, &Pred);

    if(Loc != -1) {

      if(strcmp(HashList.List[Loc].password, userElem.password) == 0) {
...
void BuildHashList(HashListType *HList){
  ...
  FILE *input = fopen("I5f6.txt", "r");

  while(TRUE) {

    ListElm AnItem;

    int nscan = fscanf(input, "%s %s", AnItem.RecKey, AnItem.password);
